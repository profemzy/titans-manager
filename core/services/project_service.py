...
class ProjectService(BaseService[Project]):
    ...
    def get_financial_summary(self, project: Project) -> Dict[str, Decimal]:
        ...
        return {
            "total_income": project.incomes.aggregate(total=Sum("amount"))["total"]
            or Decimal("0"),
            "total_expenses": project.expenses.aggregate(total=Sum("amount"))["total"]
            or Decimal("0"),
    ...
    def search_projects(self, **filters) -> List[Project]:
        ...
        if "date_range" in filters:
            ...
            queryset = queryset.filter(
                start_date__gte=date_range["start"], end_date__lte=date_range["end"]
            )
